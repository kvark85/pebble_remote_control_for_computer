...
static void inbox_received_callback(DictionaryIterator *iterator, void *context) {
  ...
  if(command_tuple && answer_tuple) {
    snprintf(command_buffer, sizeof(command_buffer), "%s", command_tuple->value->cstring);
    snprintf(answer_buffer, sizeof(answer_buffer), "%s", answer_tuple->value->cstring);
  }

  // Assemble full string and display
  snprintf(result_layer_buffer, sizeof(result_layer_buffer), "%s, %s", command_buffer, answer_buffer);
